...
describe('resetStuckProcessingRows — orphan claim cleanup', () => {
  ...
  // retries hit MAX_TRIES=5 and the row was marked 'failed' — upstream's old
  // handleRecurrence only re-armed `status='completed'` rows, so the series
  // died. v2.3.0's handleRecurrence re-arms `status IN ('completed','failed')`
  // (getCompletedRecurring includes failed), so sealing the row as 'failed'
  // while KEEPING its recurrence is what preserves the series — the next sweep
  // tick arms the next occurrence. This test guards that MAX_TRIES seals the
  // row as failed without deleting its recurrence (which would strand it).
  it('seals a recurring task as failed at MAX_TRIES but keeps its recurrence so the series re-arms', async () => {
    const { inDb, outDb } = makeSessionDbs();
    const claimedAt = new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString();
    ...
    // Sealed as failed for the audit trail, recurrence preserved so the
    // next sweep's handleRecurrence re-arms the series.
    const failed = inDb.prepare('SELECT status, recurrence FROM messages_in WHERE id = ?').get('task-recur') as {
      status: string;
      recurrence: string | null;
    };
    expect(failed.status).toBe('failed');
    expect(failed.recurrence).toBe('15 8-22 * * *');
  });
});
