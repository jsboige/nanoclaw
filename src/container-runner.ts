...
import { passthroughEnv } from './env-passthrough.js';
...
export function composeSessionSpec(input: ComposeSessionSpecInput): SessionSpec {
  ...
  // [PATCH-myia #1/#6/#38/#44/#39] Host env passthrough — single source of
  // truth in src/env-passthrough.ts; mountPolicy() lists the same keys so
  // validateSpec exempts exactly what this call injects.
  Object.assign(env, passthroughEnv());
  ...
  const agent: ContainerSpec = {
    ...
    command: ['bash', '-c'],
    // [PATCH-myia] Keep the CA-bundle setup the image entrypoint performs
    // (container/entrypoint.sh): the dynamic spawn bypasses entrypoint.sh, and
    // the OneCLI SDK's host-side CA bundler bails on Windows — native binaries
    // (gh, curl, git, Python) need a bundle combining the system roots with
    // the gateway CA, built here in the container instead.
    args: [
      [
