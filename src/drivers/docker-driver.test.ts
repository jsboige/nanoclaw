...
// [PATCH-myia #22] The readiness probe retries through transient Docker pipe
// failures instead of failing FATAL on the first miss — a Docker Desktop
// restart hiccup on Windows must not trip the startup circuit breaker.
describe('ensureReady — [PATCH-myia #22] retry on transient failures', () => {
    ...
    cli.responses = [
      {
        match: /^info$/,
        run: () => {
          if (misses-- > 0) throw new Error('error during connect: open //./pipe/docker_engine');
          return '';
        },
      },
    ];
