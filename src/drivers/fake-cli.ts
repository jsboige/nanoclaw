...
export class FakeCli implements Cli {
  ...
  // `run` ([PATCH-myia #22] tests): dynamic per-call behavior for scripted
  // fail-N-times-then-succeed sequences static entries can't express.
  responses: Array<{ match: RegExp; output?: string; throws?: Error | string; run?: () => string }> = [];
