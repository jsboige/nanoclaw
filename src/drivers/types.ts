...
export interface MountPolicy {
  ...
  /**
   * [PATCH-myia #1] Exact env keys this install forwards from the host
   * environment into agent containers (see src/env-passthrough.ts — the
   * single source of truth both sides consume). Upstream's invariant is that
   * no credential-shaped key rides the composed env lane: credentials arrive
   * via the gateway at egress. This fork deliberately bypasses the gateway
   * for z.ai/GitHub (PATCHES.md#2), so those keys are policy-sanctioned
   * here — listed keys skip the name check below; credential VALUES from any
   * unlisted source are still refused on every lane.
   */
  passthroughEnvKeys?: readonly string[];
}
...
export function validateSpec(spec: SessionSpec, policy: MountPolicy, capabilities?: DriverCapabilities): void {
  ...
  for (const container of spec.containers) {
    ...
    for (const [key, value] of Object.entries(container.env)) {
      // [PATCH-myia #1] Fork passthrough lane — keys listed in the install's
      // MountPolicy skip the name check; see MountPolicy.passthroughEnvKeys.
      if (policy.passthroughEnvKeys?.includes(key)) continue;
