...
export class DockerSessionDriver implements SessionDriver {
  ...
  async ensureReady(retryDelaysMs: number[] = RUNTIME_READY_RETRY_DELAYS_MS): Promise<void> {
    // [PATCH-myia #22] Retry schedule around upstream's single-shot FATAL
    // probe. On Windows the `\\.\pipe\docker_engine` named pipe briefly
    // disappears during Docker Desktop restarts, WSL kernel churn, or
    // resource pressure (observed 2026-09-06→07: a Docker Desktop death put
    // the host in a 10-minute FATAL crash-loop, each restart hitting a cold
    // daemon). The one-shot probe turns every such hiccup into a FATAL exit,
    // which trips the startup circuit breaker into 5-15min backoff and makes
    // the host fail to re-attach for far longer than the underlying Docker
    // outage. Budget ~180s — a Docker Desktop restart hiccup is typically
    // 30-90s. Exhausting the schedule rethrows the probe's final error, so
    // the failure shape (banner included) is exactly upstream's.
    // Optional schedule param: tests inject [0, …] to skip the real sleeps.
    let lastErr: unknown;
...
/**
 * [PATCH-myia #22] Retry schedule for the readiness probe in `ensureReady`.
 * Index 0 = first attempt (no delay). Total budget ~180s. Exported for tests.
 */
