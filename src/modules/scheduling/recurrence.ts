...
export async function handleRecurrence(inDb: InboundMailbox, session: Session): Promise<void> {
  ...
  for (const msg of recurring) {
    try {
      ...
      // [PATCH-myia #34-adjacent] A cron string that can't parse never fixes
      // itself — upstream's bare log means the malformed series re-throws on
      // every sweep tick. Clearing the recurrence stops the every-tick spam
      // and lets the agent see the failure once (observed: "0 21-5 * * *"
      // re-threw every minute for ~16h, blocking the night shift). Carried
      // fix, proposed upstream as nanocoai/nanoclaw#3247.
      try {
