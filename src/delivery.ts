...
// [PATCH-myia] transient readonly skip — see the catch in drainSession.
...
// [PATCH-myia #21/#46] reaction-swallow predicates — see the catch in drainSession.
...
async function drainSession(session: Session): Promise<void> {
  ...
  try {
    ...
    // path leaves a `-journal` file SQLite can't recover from a readonly
    // open. Skip the tick quietly — the next active poll (~1s) sees a clean
    // DB; an error line per tick for the whole commit window is pure spam.
  ...
  for (const msg of pending) {
    try {
      ...
      // swallow. The bridge-level swallow (chat-sdk-bridge.ts) is the primary
      // defense; this delivery-layer net catches any addReaction call that
      // bypasses the bridge swallow (e.g. a future adapter that routes
      // reactions through a different code path, or a regression in the
      // bridge). A "react not found" error means the target message is gone —
      // the reaction is a UX cue, not load-bearing, so we mark the row
      // delivered instead of burning the retry budget and spamming the
