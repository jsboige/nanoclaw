...
// [PATCH-myia] The bound exercises the REAL production thresholds (64KB / 256KB)
// rather than injected ones: the defect these guard against is a file that grew
// past the shipped limits, so a test that moves the limits proves nothing.
// Carried from claude-md-compose.test.ts (66ff746c).
describe('CLAUDE.local.md injection bound', () => {
  function localPath(folder: string): string {
    return path.join(groupDirOf(folder), 'CLAUDE.local.md');
  }
  function overflowFiles(folder: string): string[] {
    return fs.readdirSync(groupDirOf(folder)).filter((f) => f.startsWith('CLAUDE.local.overflow-'));
  }

  it('leaves a file under the warn threshold untouched and silent', async () => {
    const ag = await seed('ag-small', 'bound-small');
    await compose(ag);
    const body = 'rule\n'.repeat(200);
  ...
  it('warns past the warn threshold WITHOUT mutating the file', async () => {
    const ag = await seed('ag-warnb', 'bound-warn');
    await compose(ag);
  ...
  it('rotates past the bound, preserving the previous contents verbatim', async () => {
    const ag = await seed('ag-rotate', 'bound-rotate');
    await compose(ag);
    ...
    expect(fs.readFileSync(path.join(groupDirOf(ag.folder), archives[0]), 'utf-8')).toBe(body);
