...
// [PATCH-myia] `CLAUDE.local.md` is the agent-owned half of the injected
// instruction surface: Claude Code auto-loads it from the group directory into
// the system prompt on every turn. Nothing bounds its growth — its own prose
// guard ("KEEP THIS SECTION SMALL") is addressed to the agent that must not
// violate it, and has been violated twice: 2026-07-19 (~280KB of DONE
// cycle-logs) and 2026-08-26 (388KB, six days of silent autocompact-thrash
// before anyone looked, issue #3273). A rule enforced only by the layer it
// constrains does not bound; it defers. Carried from claude-md-compose.ts
// (66ff746c) when this module replaced it.
//
...
/**
 * [PATCH-myia] Bound the agent-owned `CLAUDE.local.md` at spawn. Warns past
