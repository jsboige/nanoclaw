...
- **File:** `container/agent-runner/src/poll-loop.ts` (new `evaluateToolStuckBudget` pure helper exported for tests, new `toolStuckHandle` `setInterval` paired with `clearInterval` in the existing finally block); `container/agent-runner/src/db/connection.ts` (new `getContainerToolInFlight` reader); `container/agent-runner/src/providers/claude.ts` (`abortController` in SDK options + `controller.abort()` + `sdkResult.close()` in `abort()`).
- **Summary:** A third per-query watchdog complementing PATCH #26 (SDK-event silence while awaiting result) and PATCH #27 (idle queue while not awaiting). This one gates on `container_state.tool_started_at` from the outbound DB — the row populated by claude.ts's PreToolUse hook on every tool call. Every `TOOL_STUCK_CHECK_INTERVAL_MS=30s`, if `current_tool != null` AND `now - tool_started_at > budget` (with `budget = max(declared_timeout_ms * 1.5, TOOL_STUCK_DEFAULT_MS=5min)`), force `query.abort()`, clear stale `container_state`, reset all claims (initial batch + pending follow-up acks + queued follow-ups) so the outer loop respawns a fresh query and re-picks the backlog. Reuses the existing `stalledAborted` flag so the outer loop's skip-markCompleted branch covers this case. **2026-05-19 follow-up:** `abort()` originally set `aborted=true` + `stream.end()` — insufficient against a hung MCP call because `for await (const msg of sdkResult)` is suspended awaiting a message that will never arrive (the `if (aborted) return;` check only runs when a new message arrives). Now `abort()` also calls `abortController.abort()` (signal-propagated to the SDK) AND `sdkResult.close()` (per SDK doc: "forcefully ends the query, cleaning up all resources including pending requests, MCP transports, and the CLI subprocess"). Verified after the 2026-05-19T20:15Z zombie (`mcp__roo-state-manager__roosync_dashboard` stuck 2h DESPITE PATCH #28 watchdog detecting it).
...
- **Lines:** ~110 (constants + 22-line pure helper + 11-line getContainerToolInFlight + 70-line setInterval + clearInterval in finally + 11 unit tests + 15-line abortController+close() in claude.ts).
