...
async function runPollLoopWithTimeout(provider: MockProvider, signal: AbortSignal, timeoutMs: number): Promise<void> {
  return Promise.race([
    // Forward `signal` so the loop actually stops when the test aborts.
    // Without it the loop never observes the abort and keeps polling the
    // module-singleton session DB forever, corrupting every later container
    // test (e.g. clobbering the shared `mock` continuation that
    // stale-session-recovery in integration.test.ts asserts on).
