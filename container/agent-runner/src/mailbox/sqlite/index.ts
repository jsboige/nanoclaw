...
export class SqliteAgentMailbox implements AgentMailbox {
  ...
  // [PATCH-myia #18] Release processing claims for a retry after a push-mode
  // stall (see db/messages-in.ts). Same table clearStaleProcessingAcks owns.
  resetProcessingAcks(ids: string[]): void {
...
// [PATCH-myia #9] task_run_logs — v1 task-history surface re-homed into the
// sqlite driver so no raw SQL lives in src/db (the architecture test greps
// that directory for direct DB access). Public shape preserved via the
// src/db/task-run-logs.ts facade.
...
export function listTaskRuns(opts: { taskOrSeriesId?: string; limit?: number } = {}): TaskRunLog[] {
