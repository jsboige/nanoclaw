...
// Delegated to the mailbox seam (sqlite driver) so no raw SQL lives outside
// the driver — the architecture test greps src/db/ for direct DB access.
export function resetProcessingAcks(ids: string[]): void {
  getAgentMailbox().operations.resetProcessingAcks(ids);
}
