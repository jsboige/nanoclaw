  ...
  try {
    for await (const event of query.events) {
      ...
      if (event.type === 'init') {
        ...
        if (event.text) {
          // [PATCH-myia #19] Pass isError so dispatchResultText's routing-source
          ...
          const { sent, hasUnwrapped, taskBlocks } = dispatchResultText(event.text, routing, event.isError === true);
